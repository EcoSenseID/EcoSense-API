...
const getCampaign = async (request, response) => {
    ...
    try {
        ...
        pool.query(queryString, (error, results) => {
            ...
            if (categoryId) {
                const campaignsList = results[1].rows.filter(data => (data.category || []).includes(categoryId));
            } else {
            ...
            if (categoryId) {
                response.status(200).json({
                    ...
                    timestamp: new Date(),
                    campaigns: results[1].rows.filter(data => (data.category || []).includes(parseInt(categoryId))).map(data => ({
                        id: data.id,
                ...
                response.status(200).json({
                    ...
                    message: "Campaigns fetched successfully",
                    timestamp: new Date(),
                    campaigns: results[1].rows.map(data => ({
                        id: data.id,
                        posterUrl: data.poster_url,
                        title: data.title,
                        description: data.description,
